  ...
  var Article = sequelize.define(
    'article',
    {
          ...
          textLength(value) {
            ...
            let titleMinLength =
              (this.config &&
                ...
                this.config.articles.titleMinLength) ||
              10;
            let titleMaxLength =
              (this.config &&
                this.config.articles &&
                this.config.articles.titleMaxLength) ||
              50;
        ...
        validate: {
          textLength(value) {
            ...
            let summaryMinLength =
              (this.config &&
                this.config.articles &&
                this.config.articles.summaryMinLength) ||
              20;
            let summaryMaxLength =
              (this.config &&
                this.config.articles &&
                this.config.articles.summaryMaxLength) ||
              140;
        ...
        validate: {
          textLength(value) {
            ...
            let descriptionMinLength =
              (this.config &&
                this.config.articles &&
                this.config.articles.descriptionMinLength) ||
              140;
            let descriptionMaxLength =
              (this.config &&
                this.config.articles &&
                this.config.articles.descriptionMaxLength) ||
              5000;
      ...
      location: {
        ...
        allowNull: !(
          config.articles &&
          config.articles.location &&
          config.articles.location.isMandatory
        ),
      },
      ...
      validate: {
        ...
        validExtraData: function (next) {
          ...
          let configExtraData =
            ...
            self.config.articles.extraData;
            ...
            if (config) {
              ...
              if (
                !(
                  self.config &&
                  self.config.articles &&
                  self.config.articles.extraDataMustBeDefined === false
                )
              ) {
                errors.push(
                  `article.extraData is not configured in site.config`
                );
              }
            }
          }
  ...
  Article.getRunning = function (sort, extraScopes) {
    ...
    return this.scope(...scopes)
      ...
      .then((articles) => {
        ...
        ranked.forEach((article) => {
          article.ranking =
            article.status == 'DENIED' ? -10000 : article.yes - article.no;
        });
        ...
        db.Site.findByPk(instance.siteId)
          ...
          .then((site) => {
            ...
            if (instance.changed('startDate')) {
              var duration =
                (instance.config &&
                  instance.config.articles &&
                  instance.config.articles.duration) ||
                90;
