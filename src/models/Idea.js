...
module.exports = function (db, sequelize, DataTypes) {
    ...
    'idea',
    {
      ...
      endDate: {
        ...
        get: function () {
          ...
          var duration =
            (_config && _config.ideas && _config.ideas.duration) || 90;
          if (
            this.site &&
            this.site.config &&
  ...
  async function beforeValidateHook(instance, options) {
    ...
    let canEditAfterFirstLikeOrArg =
      (siteConfig && siteConfig.canEditAfterFirstLikeOrArg) || false;
    if (
      ...
      let firstLikeSubmitted = await db.Vote.count({
        where: { ideaId: instance.id },
      });
      let firstArgSubmitted = await db.Argument.count({
        where: { ideaId: instance.id },
      });
