...
module.exports = function (db, sequelize, DataTypes) {
  var Idea = sequelize.define(
    'idea',
    {
      ...
      endDate: {
        ...
        get: function () {
          ...
          if (
            this.site &&
            this.site.config &&
            this.site.config.votes &&
            this.site.config.votes.isActiveTo
          ) {
        ...
        validate: {
          ...
          textLength(value) {
            ...
            let titleMinLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.titleMinLength) ||
              10;
            let titleMaxLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.titleMaxLength) ||
              50;
        ...
        validate: {
          ...
          textLength(value) {
            ...
            let summaryMinLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.summaryMinLength) ||
              20;
            let summaryMaxLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.summaryMaxLength) ||
              140;
        ...
        validate: {
          ...
          textLength(value) {
            ...
            let descriptionMinLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.descriptionMinLength) ||
              140;
            let descriptionMaxLength =
              (this.config &&
                this.config.ideas &&
                this.config.ideas.descriptionMaxLength) ||
              5000;
      ...
      location: {
        ...
        allowNull: !(
          config.ideas &&
          config.ideas.location &&
          config.ideas.location.isMandatory
        ),
        ...
        get: function () {
          var minimumYesVotes =
            (this.site &&
              this.site.config &&
              this.site.config.ideas &&
              this.site.config.ideas.minimumYesVotes) ||
            config.get('ideas.minimumYesVotes');
    ...
    },
    {
      ...
      validate: {
        ...
        validExtraData: function (next) {
          ...
          let configExtraData =
            self.config && self.config.ideas && self.config.ideas.extraData;
                ...
                if (typeof validated[key] == 'undefined') {
                  if (
                    !(
                      self.config &&
                      self.config.ideas &&
                      self.config.ideas.extraDataMustBeDefined === false
                    )
                  ) {
  ...
  Idea.scopes = function scopes() {
    ...
    return {
      ...
      mapMarkers: {
        attributes: ['id', 'status', 'location', 'position'],
        where: sequelize.or(
          {
        ...
        filterKeys.forEach((filter, i) => {
                ...
                filterValue.forEach((value, key) => {
                  ...
                  conditions[Sequelize.Op.and].push({
                    [Sequelize.Op.and]: sequelize.literal(
                      `extraData->${escapedKey}=${escapedValue}`
                    ),
                  });
                });
              } else {
          ...
          if (filtersExclude) {
            ...
            if (excludeFilterValue) {
              if (filter.extraData) {
                excludeFilterValue = Array.isArray(excludeFilterValue)
                  ? excludeFilterValue
                  : [excludeFilterValue];
      ...
      includeArguments: function (userId) {
        return {
          ...
          order: [
            sequelize.literal(
              `GREATEST(0, \`argumentsAgainst.yes\` - ${argVoteThreshold}) DESC`
            ),
            sequelize.literal(
              `GREATEST(0, \`argumentsFor.yes\` - ${argVoteThreshold}) DESC`
            ),
      ...
      includeVoteCount: {
        attributes: {
          include: [voteCount('yes'), voteCount('no')],
        },
      },

      includeArgsCount: {
        attributes: {
          include: [argCount('argCount')],
        },
      },
      ...
      summary: {
        attributes: {
          include: [voteCount('yes'), voteCount('no'), argCount('argCount')],
          exclude: ['modBreak'],
        },
      },
      withUser: {
  ...
  Idea.getRunning = function (sort, extraScopes) {
    ...
    if (config.siteId && typeof config.siteId == 'number') {
      where = {
        $and: [{ siteId: config.siteId }, ...where],
      };
    }
  ...
  let canMutate = function (user, self) {
    ...
    let canEditAfterFirstLikeOrArg =
      (config && config.canEditAfterFirstLikeOrArg) || false;
    let voteCount = self.no + self.yes;
    let argCount =
      self.argumentsFor &&
      self.argumentsFor.length &&
      self.argumentsAgainst &&
      self.argumentsAgainst.length;
        ...
        db.Site.findByPk(instance.siteId)
          ...
          .then((site) => {
            ...
            if (instance.changed('startDate')) {
              var duration =
                (instance.config &&
                  instance.config.ideas &&
                  instance.config.ideas.duration) ||
                90;
              if (
                this.site &&
                this.site.config &&
                ...
                duration =
                  this.site.config.ideas.automaticallyUpdateStatus.afterXDays ||
                  0;
              }
              var endDate = moment(instance.startDate)
                .add(duration, 'days')
                .toDate();
