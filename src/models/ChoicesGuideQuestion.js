...
module.exports = function (db, sequelize, DataTypes) {
  let ChoicesGuideQuestion = sequelize.define(
    'choicesGuideQuestion',
    {
      questionGroupId: {
          ...
          return new Promise((resolve, reject) => {
            if (instance.choicesGroupId) {
              db.ChoicesGuideQuestion.findByPk(instance.questionGroupId)
                ...
                .then((questionGroup) => {
                  db.ChoicesGuide.scope('includeSite')
      ...
      forSiteId: function (siteId) {
        return {
          where: {
            questionGroupId: [
              sequelize.literal(
                `select choicesGuideQuestionGroups.id FROM choicesGuideQuestionGroups INNER JOIN choicesGuides ON choicesGuides.id = choicesGuideQuestionGroups.choicesGuideId WHERE siteId = ${siteId}`
              ),
            ],
          },
        };
