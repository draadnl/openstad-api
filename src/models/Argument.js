          ...
          textLength(value) {
            ...
            let descriptionMinLength =
              (this.config &&
                ...
                this.config.arguments.descriptionMinLength) ||
              30;
            let descriptionMaxLength =
              (this.config &&
                this.config.arguments &&
                this.config.arguments.descriptionMaxLength) ||
              500;
    ...
    },
    {
      hooks: {
        ...
        afterCreate: function (instance, options) {
          db.Idea.findByPk(instance.ideaId).then((idea) => {
      ...
      forSiteId: function (siteId) {
        return {
          where: {
            ideaId: [
              sequelize.literal(
                `select id FROM ideas WHERE siteId = ${siteId}`
              ),
            ],
          },
        };
