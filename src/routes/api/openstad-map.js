  ...
  .get(function (req, res, next) {
    db.Idea.scope(...req.scope)
      ...
      .then((found) => {
        let maxMarkers =
          (req.site &&
            req.site.config.openStadMap &&
            req.site.config.openStadMap.maxMarkers) ||
          (config.openStadMap && config.openStadMap.maxMarkers) ||
          20;
...
router
  ...
  .get(function (req, res, next) {
    ...
    let polygon =
      req.site &&
      req.site.config.openStadMap &&
      req.site.config.openStadMap.polygon;

    // fallback to generic config
    polygon =
      polygon ||
      (config.openStadMap &&
        config.openStadMap.polygons &&
        ((config.openStadMap.usePolygon &&
          config.openStadMap.polygons[config.openStadMap.usePolygon]) ||
          (config.siteId && config.openStadMap.polygons[config.siteId])));
  ...
  return {
    ...
    icon: {
      url:
        ...
        idea.status == 'BUSY'
          ? '/img/idea/flag-blue.svg'
          : idea.status == 'CLOSED' || idea.status == 'DENIED'
          ? '/img/idea/flag-gray.svg'
          : '/img/idea/flag-red.svg',
      size: [22, 24],
      anchor: [4, 21],
    },
    href: `/plan/${idea.id}`,
