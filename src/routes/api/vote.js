...
router
  ...
  .all(function (req, res, next) {
    if (!(req.site && req.site.config && req.site.config.votes)) {
      return next(
        createError(403, 'Site niet gevonden of niet geconfigureerd')
      );
    }
    return next();
  });
...
router
  ...
  .all(function (req, res, next) {
    ...
    if (
      req.site.config.votes.requiredUserRole == 'anonymous' &&
    ...
    if (
      req.site.config.votes.requiredUserRole == 'member' &&
      (req.user.role == 'member' || hasModeratorRights)
    ) {
      return next();
    }

    if (
      req.site.config.votes.requiredUserRole == 'admin' &&
      hasModeratorRights
    ) {
      return next();
    }
...
router
  ...
  // heb je al gestemd
  .post(function (req, res, next) {
    db.Vote.scope(req.scope) // get existing votes for this user
      .findAll({ where: { userId: req.user.id } })
      .then((found) => {
        if (
          req.site.config.votes.voteType !== 'likes' &&
          req.site.config.votes.withExisting == 'error' &&
          found &&
          found.length
        )
      ...
      try {
        if (
          req.existingVotes.find((newVote) =>
            votes.find((oldVote) => oldVote.ideaId == newVote.ideaId)
          )
        )
          throw createError(403, 'Je hebt al gestemd');
      } catch (err) {
  ...
  .post(function (req, res, next) {
    ...
    db.Idea.findAll({ where: { id: ids, siteId: req.site.id } }).then(
      (found) => {
  ...
  .post(function (req, res, next) {
    ...
    if (
      ...
      req.votes.forEach((vote) => {
        ...
        db.Vote.findAll({ where: whereClause })
          .then((found) => {
  ...
  .post(function (req, res, next) {
    ...
    if (
      req.votes.length >= req.site.config.votes.minIdeas &&
      req.votes.length <= req.site.config.votes.maxIdeas
    ) {
      return next();
    }
  ...
  .post(function (req, res, next) {
    ...
    if (
      !(
        budget >= req.site.config.votes.minBudget &&
        budget <= req.site.config.votes.maxBudget
      )
    ) {
  ...
  .post(function (req, res, next) {
    ...
    if (
      (req.site.config.votes.minIdeas &&
        totalNoOfVotes < req.site.config.votes.minIdeas) ||
      (req.site.config.votes.maxIdeas &&
        totalNoOfVotes > req.site.config.votes.maxIdeas)
    ) {
      isOk = false;
    }
  ...
  .post(function (req, res, next) {
    ...
    switch (req.site.config.votes.voteType) {
      case 'likes':
        req.votes.forEach((vote) => {
          let existingVote = req.existingVotes
            ? req.existingVotes.find((entry) => entry.ideaId == vote.ideaId)
            : false;
          if (existingVote) {
  ...
  .post(function (req, res, next) {
    ...
    db.Vote.findAll({ where: { userId: req.user.id, ideaId: ideaIds } }) // get existing votes for this user
      .then((found) => {
