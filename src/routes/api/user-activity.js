...
router
  ...
  .get(function (req, res, next) {
    ...
    if (req.activities.length == 0)
      req.activities = ['ideas', 'articles', 'arguments', 'votes', 'sites'];

    if (
      req.query.includeOtherSites == 'false' ||
      req.query.includeOtherSites == '0'
    )
      req.query.includeOtherSites = false;
    req.includeOtherSites =
      typeof req.query.includeOtherSites != 'undefined'
        ? !!req.query.includeOtherSites
        : true;
    req.results = {};
    next();
  })
  ...
  .get(function (req, res, next) {
    ...
    return db.User.findOne({
      ...
      return user.getThisUserOnOtherSites().then((users) => {
        users.forEach((user) => {
  ...
  .get(function (req, res, next) {
    ...
    return db.Site.findAll({ where }).then(function (rows) {
  ...
  .get(function (req, res, next) {
    ...
    return db.Idea.findAll({ where }).then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('articles')) return next();
    let where = { userId: req.userIds };
    return db.Article.findAll({ where }).then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('arguments')) return next();
    let where = { userId: req.userIds };
    return db.Argument.findAll({ where }).then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('votes')) return next();
    let where = { userId: req.userIds };
    return db.Vote.findAll({ where }).then(function (rows) {
