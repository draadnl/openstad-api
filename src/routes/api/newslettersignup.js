  ...
  .all('*', function (req, res, next) {
    ...
    Object.keys(req.body).forEach((key) => {
      ...
      if (match) {
        ...
        if (typeof data.extraData[match[1]] == 'string')
          return data.extraData[match[1]].length < 256
            ? data.extraData[match[1]]
            : data.extraData[match[1]].substring(0, 255);
        if (typeof data.extraData[match[1]] == 'number')
          return data.extraData[match[1]] < 2147483647
            ? data.extraData[match[1]]
            : 0;
...
router
  ...
  .get(function (req, res, next) {
    ...
    if (typeof confirmed !== 'undefined')
      where.confirmed = !(confirmed == 'false' || confirmed == '0');
    db.NewsletterSignup.scope(...req.scope)
  ...
  .post(function (req, res, next) {
    let isActive =
      req.site &&
      req.site.config &&
      req.site.config.newslettersignup &&
      req.site.config.newslettersignup.isActive;
    if (!isActive)
      return next(
        createError(500, 'Nieuwsbrief aanmeldingen zijn momenteel gesloten.')
      );
    let confirmationUrl =
      req.site.config.newslettersignup.confirmationEmail &&
      req.site.config.newslettersignup.confirmationEmail.url;
    if (!confirmationUrl)
      return next(
        createError(
          500,
          'Configuratiefout: confirmationUrl is niet gedefinieerd. Waarschuw de site beheerder.'
        )
      );
    if (req.user && req.user.email && req.user.email !== req.parsedBody.email)
      return next(
        createError(400, 'Dat is niet het emailadres waarmee je bent ingelogd')
      );
    return next();
  })
  .post(function (req, res, next) {
    db.NewsletterSignup.findOne({
      where: { siteId: req.site.id, email: req.parsedBody.email },
    })
      .then((found) => {
        if (found) {
            ...
            found
              ...
              .then((result) => {
                return next(
                  createError(
                    400,
                    'Je bent al aangemeld. Je aanmelding is nu ook bevestigd.'
                  )
                );
              })
              .catch(next);
  ...
  .post(function (req, res, next) {
    ...
    if (req.user && req.user.email) {
      ...
      data.confirmed =
        ...
        req.site.config &&
        req.site.config.newslettersignup &&
        req.site.config.newslettersignup.autoConfirm;
      if (!data.confirmed) {
    ...
    db.NewsletterSignup.create(data)
      .then((result) => {
        res.json(result);
        if (!result.confirmed) {
...
router
  ...
  .post(function (req, res, next) {
    if (
      ...
      !req.body.confirmToken.match(/^[a-zA-Z0-9]{256}$/)
    )
      return next(createError(404, 'Token niet gevonden'));
    return next();
  })
  .post(function (req, res, next) {
    db.NewsletterSignup.findOne({
      where: { confirmToken: req.body.confirmToken, siteId: req.site.id },
    }).then((found) => {
...
router
  ...
  .post(function (req, res, next) {
    if (
      !req.body.signoutToken ||
      !req.body.signoutToken.match(/^[a-zA-Z0-9]{256}$/)
    )
      return next(createError(404, 'Token niet gevonden'));
    return next();
  })
  .post(function (req, res, next) {
    db.NewsletterSignup.findOne({
      where: { signoutToken: req.body.signoutToken, siteId: req.site.id },
    }).then((found) => {
      if (!found) return next(createError(404, 'Aanmelding niet gevonden'));
      found
        .destroy()
...
router
  ...
  .put(function (req, res, next) {
    ...
    if (
      !(
        newslettersignup &&
        newslettersignup.can &&
        newslettersignup.can('update')
      )
    )
      return next(new Error('You cannot update this newslettersignup'));
    newslettersignup
      .update(req.body)
      .then((result) => {
        res.json(result);
