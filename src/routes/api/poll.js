...
router
  ...
  .post(function (req, res, next) {
    // validations
    if (!req.site.config.polls || req.site.config.polls.canAddPolls != true)
      return next(createError(400, 'Poll toevoegen is niet toegestaan'));
    if (!req.idea.auth.canAddPoll(req.user, req.idea))
  ...
  .post(function (req, res, next) {
    ...
    db.Poll.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        db.Poll.scope('defaultScope', 'withIdea', {
...
router
  ...
  .post(function (req, res, next) {
    ...
    let pollVote;
    db.PollVote.scope('defaultScope', 'withPoll')
      .findOne({
