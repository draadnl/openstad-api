...
router.all('*', function (req, res, next) {
  ...
  var sort =
    ...
    (req.cookies['article_sort'] &&
      req.cookies['article_sort'].replace(/[^a-z_]+/i, ''));
  if (sort) {
...
router
  ...
  .post(function (req, res, next) {
    if (
      !(
        req.site.config &&
        req.site.config.articles &&
        req.site.config.articles.canAddNewArticles
      )
    )
  ...
  .post(function (req, res, next) {
    ...
    let responseData;
    db.Article.authorizeData(data, 'create', req.user, null, req.site)
      .create(data)
  ...
  .post(function (req, res, next) {
    ...
    articleInstance.setTags(req.body.tags).then((articleInstance) => {
      // refetch. now with tags
      let scope = [...req.scope, 'includeTags'];
      return db.Article.scope(...scope)
        .findOne({
...
router
  ...
  .put(function (req, res, next) {
    ...
    if (!(article && article.can && article.can('update')))
      return next(new Error('You cannot update this Article'));
  ...
  .put(function (req, res, next) {
    ...
    articleInstance.setTags(req.tags).then((articleInstance) => {
      // refetch. now with tags
      let scope = [...req.scope, 'includeTags'];
      return db.Article.scope(...scope)
        .findOne({
