...
router
  ...
  .post(function (req, res, next) {
    ...
    db.ChoicesGuide.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        res.json(result);
      })
...
router
  ...
  .get(function (req, res, next) {
    ...
    if (req.choicesguide.choicesGuideChoices) {
      json.choices = [];
      req.choicesguide.choicesGuideChoices
    ...
    if (req.choicesguide.choicesGuideQuestionGroups) {
      json.questiongroups = [];
      req.choicesguide.choicesGuideQuestionGroups
        .sort((a, b) => a.seqnr - b.seqnr)
        .forEach((group) => {
          let choices;
          if (group.choicesGuideChoices) {
            choices = [];
            group.choicesGuideChoices
              .sort((a, b) => a.seqnr - b.seqnr)
              .forEach((choice) => {
                choices.push({
                  id: choice.id,
  ...
  .put(function (req, res, next) {
    ...
    if (
      !(
        req.choicesguide &&
        req.choicesguide.can &&
        req.choicesguide.can('update', req.user)
      )
    )
      return next(new Error('You cannot update this ChoicesGuide'));
    req.choicesguide
...
router
  .route(
    '/:choicesGuideId(\\d+)(/questiongroup/:questionGroupId(\\d+))?/choice$'
  )
  .all(function (req, res, next) {
    req.scope = [{ method: ['forSiteId', req.site.id] }];
  ...
  .post(function (req, res, next) {
    ...
    db.ChoicesGuideChoice.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        res.json(result);
      })
...
router
  .route(
    '/:choicesGuideId(\\d+)(/questiongroup/:questionGroupId(\\d+))?/choice/:choiceId(\\d+)$'
  )
  .all(function (req, res, next) {
    let choicesGuideId = parseInt(req.params.choicesGuideId);
    let questionGroupId = parseInt(req.params.questionGroupId);
    if (!choicesGuideId && !questionGroupId)
      throw createError(
        404,
        'choicesGuide or choicesGuideQuestionGroup not found'
      );
    ...
    db.ChoicesGuideChoice.scope(...req.scope)
      .findOne({
        where: where,
      })
      .then((found) => {
  ...
  .put(auth.useReqUser)
  .put(function (req, res, next) {
    if (!(req.choice && req.choice.can && req.choice.can('update', req.user)))
      return next(new Error('You cannot update this Choice'));
    let data = {
      title: req.body.title,
      description: req.body.description,
      images: req.body.images,
...
router
  .route('/:choicesGuideId(\\d+)/questiongroup$')
  .all(function (req, res, next) {
    req.scope = [{ method: ['forSiteId', req.site.id] }];
  ...
  .post(function (req, res, next) {
    ...
    db.ChoicesGuideQuestionGroup.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        res.json(result);
      })
...
// ----------------------------------
router
  .route('/:choicesGuideId(\\d+)/questiongroup/:questionGroupId(\\d+)$')
  .all(function (req, res, next) {
    let choicesGuideId = parseInt(req.params.choicesGuideId);
    if (!choicesGuideId) throw createError(404, 'choicesGuide not found');
    let questionGroupId = parseInt(req.params.questionGroupId);
    if (!questionGroupId) throw createError(404, 'questionGroup not found');
    db.ChoicesGuideQuestionGroup.scope(...req.scope)
      .findOne({
  ...
  .put(function (req, res, next) {
    if (
      !(
        req.questiongroup &&
        req.questiongroup.can &&
        req.questiongroup.can('update', req.user)
      )
    )
      return next(new Error('You cannot update this Question'));
    let data = {
      answerDimensions: req.body.answerDimensions,
      title: req.body.title,
      description: req.body.description,
...
router
  .route(
    '/:choicesGuideId(\\d+)/questiongroup/:questionGroupId(\\d+)/question$'
  )
  .all(function (req, res, next) {
    req.scope = [{ method: ['forSiteId', req.site.id] }];
  ...
  .post(function (req, res, next) {
    ...
    db.ChoicesGuideQuestion.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        res.json(result);
      })
...
router
  .route(
    '/:choicesGuideId(\\d+)/questiongroup/:questionGroupId(\\d+)/question/:questionId(\\d+)$'
  )
  .all(function (req, res, next) {
    let questionGroupId = parseInt(req.params.questionGroupId);
    if (!questionGroupId) throw createError(404, 'questionGroup not found');
    ...
    db.ChoicesGuideQuestion.scope(...req.scope)
      .findOne({
  ...
  .put(function (req, res, next) {
    if (
      !(
        req.question &&
        req.question.can &&
        req.question.can('update', req.user)
      )
    )
      return next(new Error('You cannot update this Question'));
    let data = {
...
router
  ...
  .all(function (req, res, next) {
    ...
    if (!(req.choicesguide.config && req.choicesguide.config.requiredUserRole))
      return next();

    // todo: gebruik hasRole

    if (
      req.choicesguide.config.requiredUserRole == 'anonymous' &&
    ...
    if (
      req.choicesguide.config.requiredUserRole == 'member' &&
      (req.user.role == 'member' ||
        req.user.role === 'editor' ||
        req.user.role === 'moderator' ||
        req.user.role === 'admin')
    ...
    if (
      req.choicesguide.config.requiredUserRole == 'editor' &&
      (req.user.role === 'editor' ||
        req.user.role === 'moderator' ||
        req.user.role === 'admin')
    ) {
    ...
    if (
      req.choicesguide.config.requiredUserRole == 'moderator' &&
      (req.user.role === 'moderator' || req.user.role === 'admin')
    ) {
      return next();
    }
    if (
      req.choicesguide.config.requiredUserRole == 'admin' &&
      req.user.role === 'admin'
    ) {
      return next();
    }
  ...
  .post(function (req, res, next) {
    ...
    db.ChoicesGuideResult.scope(req.scope) // get existing ChoicesGuideResult for this user
      .findOne({ where })
      .then((found) => {
        if (
          req.choicesguide.config &&
          req.choicesguide.config.withExisting == 'error' &&
          found
        )
          throw new Error('Je hebt je mening al ingestuurd');
  ...
  .post(function (req, res, next) {
    ...
    if (
      req.choicesguide.config &&
      req.choicesguide.config.withExisting == 'replace' &&
      req.existingResult
    ) {
      req.existingResult
