...
const refreshSiteConfigMw = function (req, res, next) {
  ...
  const cmsUrl =
    site.config.cms && site.config.cms.url
      ? site.config.cms.url
      : 'https://' + site.domain;
...
router
  ...
  .put(function (req, res, next) {
    const site = req.results;
    if (!(site && site.can && site.can('update')))
      return next(new Error('You cannot update this site'));
  ...
  .put(function (req, res, next) {
    ...
    req.siteOAuthClients.forEach((oauthClient, i) => {
      const authUpdateUrl =
        authServerUrl + '/api/admin/client/' + oauthClient.id;
      ...
      oauthClient.config['users'] = {
        canCreateNewUsers: req.site.config.users.canCreateNewUsers,
      };
