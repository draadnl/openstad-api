...
const formatOAuthApiUrl = (path, siteConfig, which = 'default') => {
  let siteOauthConfig =
    (siteConfig && siteConfig.oauth && siteConfig.oauth[which]) || {};
  let url =
...
const formatOAuthApiCredentials = (siteConfig, which = 'default', token) => {
  ...
  return (
    'Basic ' +
    new Buffer(`${authClientId}:${authClientSecret}`).toString('base64')
  );
};
...
OAuthAPI.fetchClient = async function ({ siteConfig, which = 'default' }) {
  const oauthServerUrl = formatOAuthApiUrl(
  ...
  return fetch(oauthServerUrl, {
    headers: {
      Authorization: oauthServerCredentials,
...
OAuthAPI.updateClient = async function ({
  siteConfig,
  which = 'default',
  clientData = {},
}) {
  ...
  const oauthServerUrl = formatOAuthApiUrl(
    `/api/admin/client/${orgClientData.id}`,
    siteConfig,
    which
  );
  const oauthServerCredentials = formatOAuthApiCredentials(siteConfig, which);
...
OAuthAPI.fetchUser = async function ({
  siteConfig,
  which = 'default',
  email,
  userId,
  ...
  const oauthServerCredentials = formatOAuthApiCredentials(
    siteConfig,
    which,
    token
  );
...
OAuthAPI.createUser = async function ({
  siteConfig,
  which = 'default',
  userData = {},
}) {
...
OAuthAPI.updateUser = async function ({
  siteConfig,
  which = 'default',
  userData = {},
}) {
  ...
  let orgUserData = await OAuthAPI.fetchUser({
    raw: true,
    siteConfig,
    which,
    userId: userData.id,
  });
  let mergedUserData = OAuthUser.mergeDataForSite(
    siteConfig,
    orgUserData,
    userData
  );
...
OAuthAPI.deleteUser = async function ({
  siteConfig,
  which = 'default',
  userData = {},
}) {
  ...
  const oauthServerUrl = formatOAuthApiUrl(
    `/api/admin/user/${userData.id}/delete`,
    siteConfig,
    which
  );
  const oauthServerCredentials = formatOAuthApiCredentials(siteConfig, which);
