...
Notifications.sendMessage = function (siteId, type, action, data) {
  ...
  // get config
  db.Site.findByPk(siteId).then((site) => {
    ...
    maildata.subject =
      type == 'argument'
        ? 'Nieuwe argumenten geplaatst'
        : action == 'create'
        ? 'Nieuwe inzending geplaatst'
        : 'Bestaande inzending bewerkt';

    maildata.from =
      (myConfig.notifications &&
        (myConfig.notifications.from ||
          (myConfig.notifications.admin &&
            myConfig.notifications.admin.emailAddress))) ||
      myConfig.mail.from; // backwards compatible
    maildata.to =
      (myConfig.notifications && myConfig.notifications.to) || maildata.from;

    maildata.EMAIL = maildata.from;
    maildata.HOSTNAME =
      (myConfig.cms && (myConfig.cms.hostname || myConfig.cms.domain)) ||
      myConfig.hostname ||
      myConfig.domain;
    maildata.URL =
      (myConfig.cms && myConfig.cms.url) ||
      myConfig.url ||
      'https://' + maildata.HOSTNAME;
    maildata.SITENAME = (site && site.title) || myConfig.siteName;

    maildata.subject += ' op ' + maildata.SITENAME;

    maildata.template =
      myConfig.notifications && myConfig.notifications.template;
      ...
      .then((found) => {
        ...
        maildata.data[type] = found.map((entry) => {
          ...
          if (type == 'idea') {
            let inzendingPath =
              (myConfig.ideas &&
                myConfig.ideas.feedbackEmail &&
                myConfig.ideas.feedbackEmail.inzendingPath &&
                myConfig.ideas.feedbackEmail.inzendingPath.replace(
                  /\[\[ideaId\]\]/,
                  entry.id
                )) ||
              '/';
          ...
          if (type == 'article') {
            let inzendingPath =
              (myConfig.articles &&
                myConfig.articles.feedbackEmail &&
                myConfig.articles.feedbackEmail.inzendingPath &&
                myConfig.articles.feedbackEmail.inzendingPath.replace(
                  /\[\[articleId\]\]/,
                  entry.id
                )) ||
              '/';
