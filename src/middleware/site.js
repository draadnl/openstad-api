...
module.exports = function (req, res, next) {
  ...
  if (req.path.match('^(/doc|/dev|/accepteer-cookies|/api/repo|/api/area|/$)'))
    return next();
  ...
  if (!siteId || typeof siteId !== 'number')
    return next(
      new createError('400', 'Site niet gevonden for path: ' + req.path)
    );
  ...
  return db.Site.findOne({ where })
    .then(function (found) {
      if (!found) {
        ...
        return next(
          new createError('404', 'Site niet gevonden for siteId: ' + siteId)
        );
      }
      req.site = found;
      next();
